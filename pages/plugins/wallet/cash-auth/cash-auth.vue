<template>
    <view>
        <view v-if="data_base != null" class="padding-main">
            ...
            <view v-else>
                <view class="notice-content-blue">
                    <view>当前有效金额<text class="cr-green fw-b margin-left-sm margin-right-sm">{{user_wallet.normal_money}}</text>元</view>
                    <view class="margin-top-sm">提现最低金额<text class="cr-red fw-b margin-left-sm margin-right-sm">{{data_base.cash_minimum_amount}}</text>元起</view>
                </view>
            </view>
        </view>
        <view v-else>
